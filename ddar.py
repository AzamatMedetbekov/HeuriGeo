...
class DDAR:
            ...
            for b in self.points:
                ...
                for points, centers in ang_to_points_centers.values():
                    if len(points) >= 2 or (centers and points):
                        changed = (
                            self.force_concyclic([a, b] + points, centers) or changed
                        )
    ...
    def merge_points(self):
        ...
        for obj in itertools.chain(self.lines, self.last_small_circles, self.circles):
            for a in obj.points:
                for b in obj.points:
    ...
    def force_equal_points(self, a, b):
        ...
        self.point_subst = {x: y if y != b else a for x, y in self.point_subst.items()}
